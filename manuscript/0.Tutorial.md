...
| Abilities         |        |                                                                                                  |
| ----------------- | ------ | ------------------------------------------------------------------------------------------------ |
| Fist of Stone     | Stance | Amina turns her fists into stone which become a hand-to-hand weapon.                             |
|                   |        | Hand-to-hand: An opponent with melee weapon deals inferior damage if Amina has initiative.       |
|                   |        | Natural: Natural weapon cannot be disarmed.                                                      |
...
| Abilities    |         |                                                                                                  |
| ------------ | ------- | ------------------------------------------------------------------------------------------------ |
| Battle axe   | Weapon  | Melee: An opponent with hand-to-hand weapon deals inferior damage if Belur has initiative.       |
...
| Abilities           |           |                                                                                                  |
| ------------------- | --------- | ------------------------------------------------------------------------------------------------ |
| Sword               | Weapon    | Melee: An opponent with hand-to-hand weapon deals inferior damage if Edgar has initiative. |
|                     |           | Parry: Edgar has advantage on parry                                                              |
| Cuir Bouilli        | Armor     | This light armor reduce damage to 1d8.                                                                            |
...
| Abilities       |         |                                                                                                   |
| --------------- | ------- | ------------------------------------------------------------------------------------------------- |
| Sword           | Weapon  | Melee: An opponent with hand-to-hand weapon deals inferior damage if Karasu has initiative.       |
|                 |         | Parry: Karasu has advantage on parry                                                              |
| Bow             | Weapon  | Missile.                                                                                          |
...
| Abilities   |         |                                                                                                        |
| ----------- | ------- | ------------------------------------------------------------------------------------------------------ |
| Dagger   x2 | Weapon  | Hand-to-hand: An opponent with melee weapon deals inferior damage if Manasa has initiative.      |
|             |         | Thrown: A dagger can also be used as a thrown weapon. A thrown weapon can be recovered with an action. |
...
Then let the player interact between them by introducing themselves in character voice or by describing their character to the other players.
...
Failing to overcome a threat will cause a level of strain to the character reducing both its maximum Hit points and Energy points starting with the short term pool (Defense and Fatigue). The first level of strain reduce the pools maximum by 1 point, the second level by 2 more points and so on. Strains are cumulative, meaning that a character with 2 levels of strain reduce its pools maximum by a total of 3 points.

| Level of strains                            |   1   |   2   |   3   |   4   |
| ------------------------------------------- | :---: | :---: | :---: | :---: |
| Cumulative reduction of hit points          |  -1   |  -3   |  -6   |  -10  |
| Cumulative reduction of energy points       |  -1   |  -2   |  -3   |  -4   |
...
If the players think that their characters will need more than 8 hours to reach Amun, they can try to slow down the ogres. This can be risky because if the maneuver does not succeed, the ogres will understand the characters plan and will march faster reaching Amun one hour earlier. If the maneuver succeed, the characters will have one more hour to reach Amun.
...
1. Engage an opponent (action);
2. Attack an opponent (maneuver);
...
3. Enter a combat stance (action);
4. Occupy a vantage point (action);
5. Support a maneuver (action);

#### Engage an opponent

During combat, combatants are constantly moving around the battlefield. In order to attack a specific opponent in close combat (hand-to-hand or melee), a character must first engage the opponent by spending an action point. When engaged, combatants are more limited in their action. Each action taken require spending one additional action points per engaged enemy. Furthermore, to hinder the attack of an opponent in the defensive phase, the character must be engaged with the foe. Engagement can be broken by a successful Combat maneuver.
...
| Maneuver | Resistance   | Description                                                                         |
| -------- |  ----------- |  ---------------------------------------------------------------------------------- |
| Combat   | Reflex       | Find an opening in the opponent's defense with quick attacks, parries and ripostes. |
| Daunt    | Presence     | Cause hesitation in the defender by making flashy moves, intimidation or taunting.  |
| Feint    | Awareness    | Expose your opponent with a series of misleading attacks.                           |
| Scuffle  | Stamina      | Overwhelm the opponent with chaotic strikes involving bare fist, pummel and shield. |
| Tactics  | Intelligence | Trick a defender into making a mistake.                                             |
...
Formations are a special stance started by a leader that allows other characters to enter the same stance as the leader. All members of a formation receive the benefits of the stance as long as they focus on the formation, but if the leader lose focus, the formation is broken and everyone lose the benefit of the stance.

#### Occupy a vantage point

A battlefield can have multiple vantage points that give a benefit to a combatant that take advantage of it. To gain the benefits of a vantage point, the player must enter a stance to occupy it. As long as that character stay in that stance, no other character can use the vantage point. A character can only use one vantage point at a time.
...
Even if it is the opponents turn, characters are active by:

1. defending against an attack
2. hindering an attack
3. improving their defense

#### Defending against an attack

Each attack made by demons have a predetermined amount of successes which are reduced by the character defensive maneuver successes. The defensive maneuver is made with the appropriate resistance against the attack.

If the attack successes is reduce to zero, the character suffer no ill effect from the attack. Otherwise, the storyteller can spend the remaining successes to incapacitate the character or activate a combat effect.
...
| *Vantage points* |
| Old cart       | A character can get behind an old cart and receive cover against ranged and flying attack. Opponents attacking a character with cover is at disadvantage and deals inferior damage. |
...
| Attacks       | Successes | Reach        | Description                                                                                              |
| ------------- | --------- | ------------ | -------------------------------------------------------------------------------------------------------- |
| Club          |     1     | Melee        | ttack.                                                                                                   |
| Talons        |     1     | Hand to hand | natural attack.                                                                                          |
| Blood drain   |     1     | Hand to hand | On a Talon attack that reduced Health, the styrge regain the same number of Health than its opponent have lost. |
...
The siren captivating song is an area attack that can affect all characters. Each character defend individually against the attack.
...
| ---------------- | --------- | ------------------------------------------------------------------------------------------------------ |
| Talons           |     1     | Hand to hand | natural attack.                                                                                    |
| Captivating song |     2     | Area         | Characters in the area become captivated reducing their maximum Focus as long as the siren maintain a stance singing.
| Wail of death    |     1     | Area         | When an siren becomes defenseless, she will make a terrifying shriek piercing the ears of every foes in the area. (Daunt Attack)
...
| ------------- | --------- | --------------------------------------------------------------------------------------- |
| Talons        |     1     | Hand-to-hand | natural attack.                                                          |
| Rake          |     2     | Hand-to-hand | Talons attack against unarmed opponent.                                  |
| Snatch        |     1     | Hand-to-hand | A snatcher can use the disarm combat effect.                             |

| Abilities     | Type      | Description                                                                                |
| ------------- | --------- | ------------------------------------------------------------------------------------------ |
| Lock target   | Stance    | Increase action cost when engage with the snatcher.                                        |
